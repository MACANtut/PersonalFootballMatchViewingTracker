# database.py - добавлен метод get_club_by_name
import hashlib
import psycopg2
...
class Database:
    ...
    def ensure_connection(self):
        ...
        try:
            ...
            print(f"Соединение потеряно, переподключаюсь: {e}")
            self.connect()
    ...
    def get_club_by_name(self, club_name):
        ...
        try:
            self.ensure_connection()
            with self.conn.cursor() as cur:
                cur.execute("SELECT id, name FROM clubs WHERE LOWER(name) = LOWER(%s)", (club_name,))
                result = cur.fetchone()
                if result:
    ...
    def get_club_id_by_name(self, club_name):
        ...
        try:
            self.ensure_connection()
            with self.conn.cursor() as cur:
                cur.execute("SELECT id FROM clubs WHERE LOWER(name) = LOWER(%s)", (club_name,))
                result = cur.fetchone()
                if result:
                    return result[0]
                return None
