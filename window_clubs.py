...
from PySide6.QtGui import QPixmap, QFont, QPainter, QPalette, QBrush
import requests
...
class LeagueWidget(QWidget):
    def __init__(self, league_data, db, parent=None):
        ...
        self.clubs = []  # Список клубов из БД
        ...
        if not pixmap.isNull():
            ...
            self.logo_label.setText("Лого")
            self.logo_label.setFixedSize(40, 40)
            self.logo_label.setStyleSheet("border: 2px solid #9bb89b; background-color: #ffffff; color: #2c4c3b;")
        self.logo_label.setAlignment(Qt.AlignCenter)
    ...
    def delete_club(self, club_id, club_name):
        ...
        reply = QMessageBox.question(
            ...
            f'Вы уверены, что хотите удалить клуб "{club_name}"?',
        ...
        if reply == QMessageBox.Yes:
            ...
            if success:
                self.load_clubs()  # Перезагружаем список
                QMessageBox.information(
                    self,
                    "Успешно",
                    f'Клуб "{club_name}" успешно удален'
                )
            else:
                QMessageBox.critical(
                    self,
                    "Ошибка",
                    f"Не удалось удалить клуб: {message}"
                )
    ...
    def add_club(self):
        ...
        if dialog.exec() == QDialog.DialogCode.Accepted:
            ...
            if success:
                self.load_clubs()  # Перезагружаем список клубов
                QMessageBox.information(
                    self,
                    "Успешно",
                    f'Клуб "{club_data["name"]}" успешно добавлен в лигу "{self.league_name}"!'
                )
            else:
                QMessageBox.critical(
                    self,
                    "Ошибка",
                    f"Не удалось добавить клуб: {message}"
                )
