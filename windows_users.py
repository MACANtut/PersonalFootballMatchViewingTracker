        ...
        try:
            ...
            for row, user in enumerate(filtered_users):
                ...
                if is_blocked:
                    ...
                    """)
                    # ИСПОЛЬЗУЕМ partial ВМЕСТО ЛЯМБДЫ
                    unblock_btn.clicked.connect(partial(self.unblock_user, user_id, first_name))
                    ...
                    """)
                    # ИСПОЛЬЗУЕМ partial ВМЕСТО ЛЯМБДЫ
                    block_btn.clicked.connect(partial(self.block_user, user_id, first_name))
    ...
    def block_user(self, user_id, user_name):
        ...
        if msg_box.clickedButton() == yes_button:
            ...
            if success:
                ...
                self._setup_message_box(msg_success, f"Пользователь {user_name} успешно заблокирован.")
                msg_success.exec()
                self.load_users()
            else:
                ...
                self._setup_message_box(msg_error, f"Не удалось заблокировать пользователя: {message}")
                msg_error.exec()
    ...
    def unblock_user(self, user_id, user_name):
        ...
        if msg_box.clickedButton() == yes_button:
            ...
            if success:
                ...
                msg_success.setIcon(QMessageBox.Information)
                self._setup_message_box(msg_success, f"Пользователь {user_name} успешно разблокирован.")
                msg_success.exec()
                self.load_users()
            else:
                msg_error = QMessageBox(self)
                ...
                msg_error.setIcon(QMessageBox.Critical)
                self._setup_message_box(msg_error, f"Не удалось разблокировать пользователя: {message}")
                msg_error.exec()