...
class UsersWindow(QDialog):
    ...
    def initUI(self):
        ...
        # Настройка растяжения колонок для заполнения всей ширины
        header = self.users_table.horizontalHeader()
        
        # Устанавливаем режим растяжения для колонок, которые должны занимать всё пространство
        header.setSectionResizeMode(0, QHeaderView.Stretch)  # Имя
        header.setSectionResizeMode(1, QHeaderView.Stretch)  # Фамилия
        header.setSectionResizeMode(2, QHeaderView.Stretch)  # Логин
        header.setSectionResizeMode(3, QHeaderView.Fixed)  # Статус - фиксированная
        header.setSectionResizeMode(4, QHeaderView.Fixed)  # Действия - фиксированная
        
        # Устанавливаем фиксированную ширину для колонок Статус и Действия
    ...
    def load_users(self):
        """Загружает список пользователей из базы данных, исключая администратора"""
        try:
            if not self.db:
                return
            ...
            # Фильтруем пользователей, исключая администратора (логин 'admin')
            ...
            self.total_users_label.setText(f"Всего пользователей: {len(filtered_users)}")
